...
核心分工：**PM 做决策，AI 画图。** 图表不要求 PM 提供——AI 从 PRD 文字/表格核对各图关注点，不确定处变成选择题，PM 确认后按需生成验证图。

**不是**从 0 写 PRD / 出原型的工具；那类需求用 `prd-writer`（若已安装）。

## prd-review：技术审查 + 交互问答

1. **技术审查**：D1–D10 始终审 PRD；D11 有原型/设计稿才审；D12–D17 有图严格审，无图按该图「关注点」核对文字/表格覆盖（**缺图不扣分**）；模块完整性 ⑦⑧⑪ 两级定级
